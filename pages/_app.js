//_app은 서버로 요청이 들어왔을때 가장 먼저 실행되는 컴포넌트로 , 페이지에 적용할 공통 레이아웃의 역할
...
export default function App({ Component, pageProps }) {
  ...
  //useEffect를 사용하는경우
  // 여러가지가 있지만 부수효과(side effect)처리할 때 사용함
  // DOM관련 작업이나 이벤트 등록등 렌더링외의 작업을 처리할 때 사용
  // onClick이벤트 같은 경우는 특정요소에 처리하지만 스크롤 이벤트는 window전체 스크롤을 감지
  useEffect(() => {
    const handleScroll = () => {
    ...
    handleScroll();
    //전역에 다시호출해서 컴포넌트가 처음 로드될때 스크롤 위치 확인해서 헤더 class붙여서 출력
    ...
    // 컴포넌트가 언마운트(화면에서 사라짐)될 때 이벤트 리스너제거
    // 이유  : 윈도우에 이벤트리스너를 추가하면 해당컴포넌트가 화면에서 사라져도 윈도우에 등록된 이벤트리스넌 남아있어 불필요하게 메모리를 차지하거나 이벤트핸들러가 계속 작동할 수 있음.
  }, []); // => 여기서 대괄호는 의존성 배열임 / useEffect에 의존성 배열을 추가하지 않으면 안쪽 코드를 컴포넌트가 매번 렌더링 될 때 마다 실행시킴=> 성능저하 옴

  //----------------------------------------------------------------------
  // 필터링 -> useState를 이용해서 카테고리 상태를 _app.js에서 전역으로 관리
  // 상태를 배열로 설정

  const categories = ["전체", "가구", "조명", "홈데코", "아트"];

  const [selectedCategory, setSelecterdCategory] = useState("전체");
  // 선택된 카테고리만 상태로 관리
  ...
  return (
    <>
      <CardProvider>
        ...
        <header className={isScrolled ? "header-area scroll" : "header-area"}>
          <div className="top">
